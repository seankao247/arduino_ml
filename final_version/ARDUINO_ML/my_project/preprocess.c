...
void scale_data(float (*data), int length) {
    ...
    for (i = 0; i < new_length; i++) {
        ...
        if (upper_index >= length) {
            ...
            new_data[i] = (1 - fraction) * (data)[lower_index] + fraction * (data)[upper_index];
        }
    }
    // data正規化
...
int main() {
    ...
    {
        //C:\Users\seank\Desktop\work\ARDUINO_ML\my_project\train_data_2
        sprintf(filename, "C:\\Users\\seank\\Desktop\\work\\ARDUINO_ML\\my_project\\train_data_2\\DATA%d.txt", i); //設定檔案路徑名稱
        fp = fopen(filename,"r");
        if(fp==NULL)
        {
